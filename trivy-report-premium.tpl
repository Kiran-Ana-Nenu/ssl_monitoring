...
<style>
  :root{
    ...
    --critical:#ef4444; --high:#f59e0b; --table:#0b1220; --white:#eef2ff;
  }
  body{font-family:Inter,system-ui,Segoe UI,Roboto,Arial; margin:0; color:var(--white); background:linear-gradient(180deg,#071126 0%, #071a2b 100%); padding:24px;}
  ...
  .b-critical{background:linear-gradient(90deg,#7f1d1d,#ef4444);color:#fff}
  .b-high{background:linear-gradient(90deg,#b45309,#f59e0b);color:#111}
  .b-total{background:linear-gradient(90deg,#0ea5a6,#06b6d4);color:#021124}
  .card{background:linear-gradient(180deg, rgba(255,255,255,0.03), rgba(255,255,255,0.01)); padding:16px;border-radius:12px;box-shadow:0 6px 18px rgba(2,6,23,0.6); margin-bottom:16px}
  ...
  tbody tr:hover td{background:linear-gradient(90deg, rgba(6,182,212,0.03), transparent)}
  ...
  .pill{padding:6px 10px;border-radius:999px;background:rgba(255,255,255,0.02); color:var(--muted); font-weight:600}
...
<div class="container">
  <header>
    <div>
      ...
      <div class="meta">Target artifact: <strong>{{ .ArtifactName }}</strong></div>
    ...
    <div style="margin-left:auto" class="small">Generated by Trivy</div>
  </header>
  ...
  <div class="card">
    <div class="row">
      <div class="filters" style="width:100%">
        ...
        <div class="pill" id="filterSeverity">Showing: <strong>All</strong></div>
      </div>
    </div>
  </div>

  <!-- Results: iterate Trivy Results -->
  {{ range .Results }}
  <details open>
    <summary>
      <div class="row" style="gap:14px;align-items:center">
        <div class="target">📦 {{ .Target }}</div>
        <div class="small" style="margin-left:12px">Scanned packages: {{ len .Vulnerabilities }}</div>
      </div>
    </summary>
        ...
        <tbody>
          {{ range .Vulnerabilities }}
            <tr class="vuln-row" data-severity="{{ .Severity }}">
              <td><a href="{{ index .References 0 }}" target="_blank" style="color:var(--accent);text-decoration:none">{{ .VulnerabilityID }}</a></td>
  ...
  <footer>Tip: Use the search box to filter vulnerabilities. Criticals and Highs are highlighted.</footer>
</div>

<script>
  // Client-side JS to compute counts and do simple filtering
  (function(){
